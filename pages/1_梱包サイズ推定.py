...
【修正履歴】
2026.4.14   JANコード入力欄を追加
            get_combined_estimate() を使いRAGあり・RAGなしを同時表示する2カラムレイアウトに変更
            推奨バナー表示を追加 (RAG推奨→緑、LLM推奨→黄)
            類似商品一覧に size_type と package_* 列を追加
            推定結果の表示をst.metricからmarkdownテーブルに変更 (数値の切り捨て表示を回避)
            サイドバーのユーザー名取得方法を修正 (session_state['user']['username'] → session_state.get('username',''))
            梱包重量の単位表示を g → kg に修正
2026.4.16   カテゴリフィルタ状態をUIに表示
            (フィルタ有効時: 🔍カテゴリ内検索、フォールバック時: ⚠️全カテゴリ検索 をキャプションで表示)
2026.4.18   _fmt() / _show_metrics() をモジュールレベルに移動 (条件ブロック内定義を解消)
            メーカーフィルタ追加に伴いフィルタ状態キャプションを更新
            (filtered が str になり "category+manufacturer" / "category" / "manufacturer" / "" を表示)
            メーカー選択時にカテゴリを連動絞り込み (MANUFACTURER_CATEGORY_MAP を使用)
            メーカー未選択時はカテゴリ・サブカテゴリを disabled にしてメーカー先入力を促す
            メーカーリスト先頭に「不明」を追加 (選択時はカテゴリ・サブカテゴリのみで検索可能)
            シリーズをドロップダウン化 (メーカー＋カテゴリ確定後に SERIES_MAP で選択肢を絞り込み)
            「不明」メーカー選択時は検索時に空文字に正規化 (フィルタ・クエリに混入しないよう対策)
"""
...
# ── 入力フォーム ──────────────────────────────────────────────────────────────
st.subheader("商品情報を入力してください")
st.info("入力項目が多いほど検索精度が上がります。")
